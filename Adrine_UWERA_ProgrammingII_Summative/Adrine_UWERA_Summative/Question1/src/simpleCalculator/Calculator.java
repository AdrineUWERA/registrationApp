...
public class Calculator implements ActionListener{
	...
	public Calculator() {
		...
		functionsAndOperatorsButtons.add(negativeButton = new JButton("(-)"));
		...
		// add the input field and result field into the execution screen panel
		...
		clearButton.setBounds(50,450,120,50);
		//sets the background color of the clear button to dark orange-yellow color
		clearButton.setBackground(new Color(179, 68, 20));
		// creates a border which is dark gray and border width of 2 and rounded borders
		Border bigButtonborder = BorderFactory.createLineBorder(Color.DARK_GRAY, 2);
		// sets the border of the clear button to the bolder created above
		clearButton.setBorder(bigButtonborder);
		
		// sets bound for the equal button because it will appear more bigger than the other buttons
		equalButton.setBounds(230,450,120,50);
		//sets the background color of the equal button to dark orange-yellow color
		equalButton.setBackground(new Color(179, 68, 20));
		// sets the border of the equal button to the bolder created above
		equalButton.setBorder(bigButtonborder);
		...
		calculatorPanel.add(negativeButton);
		...
		// adds the equal button to the frame of the calculator
	...
	public void actionPerformed(ActionEvent event) {
		try {
			...
			// checks if the source of the action event is the equalButton
				...
				// checks if there is an ongoing operation and if there is an operator
				if (!resultField.getText().isEmpty() & !operator.equals("")) {
					// checks if the operation is to be repeated i.e the result field already has "=" sign
					// this will help when the user wants to keep for instance adding same number to the 
					// result obtained by previous operation
					if ((resultField.getText().charAt(resultField.getText().length()-2) == '=')){
						// gets the index of the operator
						int indexOfOperator = resultField.getText().indexOf(operator, 2);
						// gets the index of the "=" sign
						int indexOfEqual = resultField.getText().indexOf('=');
						// gets the second number in the previous operation
						String subStr = resultField.getText().substring(indexOfOperator+1, indexOfEqual-1);
						// sets the second number to the same second number as previous operation
						secondNumber = Double.parseDouble(subStr);
					}
					else {
			...
			// checks if the source of the action event is the negativeButton
			if(event.getSource() == negativeButton) {
				// gets the number to be converted to its negative
				double negativeNumber = Double.parseDouble(inputField.getText());
				// converts the number to negative number
				negativeNumber*=-1;
				// puts the number into the inputfield
				inputField.setText(String.valueOf(negativeNumber).replaceAll("\\.?0*$", ""));
			}
